/* =====================================================================
 *  control_as5600_mpu6050_adaptive.ino  (versión con PWM máx=14 y ref relativa)
 *  Firma: Nadia
 *  HW: Arduino UNO + AS5600 + MPU6050 + L298N (IN1=7, IN2=8, ENA=9)
 *  Controles: PID | HINF | SMC | ADAPTIVE_PID
 *  Perfiles de referencia: STEP | RAMP | PULSET (todas relativas al ángulo inicial)
 * ===================================================================== */
#include <Wire.h>
#include <math.h>
...
/*** Ref. SIEMPRE a 60° (puedes cambiar abajo LOCK_REF_60) ***/
float ref_deg = 60.0f, ramp_deg_s=10.0f, pulse_amp_deg=15.0f, pulse_T=1.0f;
const bool LOCK_REF_60 = true;  // true: fuerza 60° aunque envíen REF por serial
...
/*** Estado sensores ***/
float offset_rad = 0.0f;   // cero del AS5600 al encender
float theta_prev = 0.0f;
float omega_est  = 0.0f;
float gyroZ_prev = 0.0f;

/*** Ángulo inicial de la PRUEBA (para que la ref sea relativa) ***/
float theta_run0 = 0.0f;   // se toma al iniciar el ensayo
...
/*** Actuación: duty normalizado [-1,1] -> PWM LIMITADO A 14 CUENTAS ***/
void driveDuty(float duty){
  duty = fmaxf(fminf(duty,1.0f),-1.0f);
  int counts = (int)(fabsf(duty) * PWM_MAX_COUNTS + 0.5f);  // <= 14
  if (duty >= 0){
    digitalWrite(PIN_IN1, HIGH);
    digitalWrite(PIN_IN2, LOW);
    analogWrite(PIN_ENA, counts);
  } else {
...
void parseLine(const String& s){
  ...
  // Formato: CFG,TS=x,T=y,MODE=...,PROF=...,REF=,RAMPS=,PAMP=,PT=
  ...
  if (!LOCK_REF_60){ // solo si permites cambiarlo por serial
    int ir = s.indexOf("REF=");   if(ir>0){ ref_deg = s.substring(ir+4).toFloat(); }
  } else {
    ref_deg = 60.0f; // forzar 60°
  }
...
/*** Referencias ABSOLUTAS respecto al ángulo inicial de la prueba ***/
float gen_ref_abs(float t, float theta0){
  if (prof==P_STEP)  return theta0 + (ref_deg*DEG2RAD);               // fijo a +60°
  if (prof==P_RAMP)  return theta0 + (ramp_deg_s*DEG2RAD)*t;          // rampa desde theta0
  if (prof==P_PULSET){
    float phase = fmodf(t, pulse_T);
    float A = pulse_amp_deg*DEG2RAD;                                  // ±A alrededor de theta0
    return theta0 + ((phase < pulse_T/2)? +A : -A);
  }
  return theta0;
}
...
void setup(){
  ...
  // Cero del AS5600 (offset de encendido)
...
void loop(){
  ...
  if (!running){
    if (TS>0 && TEST_TIME>0){
      // === CAPTURA ANGULO INICIAL DE LA PRUEBA ===
      theta_run0 = wrapToPi(as5600Rad() - offset_rad);
  ...
  omega_est = 0.6f*omega_est + 0.4f*gyroZ;      // mezcla simple (más gyro)
  float omegadot = (gyroZ - gyroZ_prev)/TS;

  // Referencia absoluta relativa a theta_run0
